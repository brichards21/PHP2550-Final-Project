...
unique(isolates_df$amr_genotypes)[1:30]
...
mean_sim_func <- function(string, clust_val) {
 purrr::map_dbl(seq_along(string), function(i) {
    if (clust_val[i] == clust_val[-i])
  {
    sim <- stringdist::stringsim(string[i], string[-i][!is.na(string[-i])])
  }
     mean(sim)
   })
