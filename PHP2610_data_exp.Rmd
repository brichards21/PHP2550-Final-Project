...
Let's start by assessing the missingess in our data. 
...
We'll start to get a good idea of our variable distributions, starting with \textbf{strain}. There are 15,348 unique strains represented in our dataset, meaning that 95.6% of the isolates in our data are unique. We may want to consider dropping this variable as a result.
...
Next, we'll look at \textbf{collection date} to see what time periods our data spans. Since this variable differs in terms of the specificity of the dates, i.e., some values only record year while others record the exact month and day as well, we'll reduce this variable to only the year for all records to consistentcy purposes.
...
prop_year_bact <- prop.table(table(isolates_df$organism_group, isolates_df$collection_date), 1) %>%
  `*`(100) %>% round(2)
...
prop_year_bact[, -1] <- matrix(paste0(as.matrix(prop_year_bact[, -1]), '%'), nrow=3)
...
kable(prop_year_bact, caption = "Prop. of Year Represented Per Bacteria") %>%
  kable_styling()

```


Interestingly, Salmonella is the only bacteria that retains isolate data for 2002-2008 (\textbf{Table 2}). Data starts to be retained for E.coli and Shigella in 2009 but shows the most records in 2017 and thereafter. Data collection for isolates for Campylobacter started in 2016 and retained records up to and including the year of 2020.

(say more)

Next, let's look at the distribution of serovar within each bacteria of interest. Upon further inspection, we see that we only have non-missing serovar information for Salmonella. There 150 distinct serovars in our Salmonella data. To reduce the number of groupings in simply, values will only be distinguished if they represent over 2% of the isolates. Remaining serovar categories will be grouped into an 'Other' classification. 
...
With this new subclassified serovar column, we are left with 13 distinct groupings. 


The AST phenotypes variable is currently one long string with all of the recorded antibiotics that each isolate is resistant, susceptible, intermediate in response, and susceptible-dose dependent to. We will separate and classify this variable into four new variables (one for each of the drug responses mentioned) and list the corresponding antibiotics for each category.
...
In order to assess this, we will compute the similarity scores between the drugs that isolates within the same clusters have. To do this, we'll use the `levenshteinSim` function in the `RecordLinkage` package in R.
...
# what I want is, if you have the same SNP cluster as me, compute score
# average similarity with everything else in your cluster

# get vector of everything else in your cluster
mean_sim_func <- function(df, var) {

 
   
   purrr::map_dbl(seq_along(1:nrow(df)), function(i) {
     
     df2 <- df %>%
     filter(snp_cluster == df[i,]$snp_cluster)
   
   string <- df2[, var]
   
    sim <- stringdist::stringsim(string[i], string[-i])
    mean(sim)
  })
}
   

isolates_df[isolates_df$organism_group == "Campylobacter jejuni",] %>% 
  mutate(dup = mean_sim_func(isolates_df[isolates_df$organism_group == "Campylobacter jejuni",], "suscept"))

#     
#   for (j in 1:nrow(df)){
#    if (!is.na(df[i,]$snp_cluster) & !is.na(df[j,]$snp_cluster) & df[i,]$snp_cluster == df[j,]$snp_cluster & i != j)
#    {
#      sim <- c(sim, stringsim(df[i, var], df[j, var]))
#    }
#   }


campy_similarity <- isolates_df[isolates_df$organism_group == "Campylobacter jejuni",]

campy_similarity  <- mean_sim_func(campy_similarity, "suscept")
...
salmonella_similarity <- isolates_df[isolates_df$organism_group == "Salmonella enterica",] %>%
  mutate(suscept_sim = mean_sim_func(suscept),
         resist_sim = mean_sim_func(resist),
         inter_sim = mean_sim_func(inter),
         sdd_sim = mean_sim_func(suscept_dose_dep)
         )


ecoli_similarity <- isolates_df[isolates_df$organism_group == "E.coli and Shigella",] %>%
  mutate(suscept_sim = mean_sim_func(suscept),
         resist_sim = mean_sim_func(resist),
         inter_sim = mean_sim_func(inter),
         sdd_sim = mean_sim_func(suscept_dose_dep))
...
campy_similarity <- isolates_df[isolates_df$organism_group == "Campylobacter jejuni",] %>%
  mutate(suscept_sim = ifelse(!is.na(suscept), mean_sim_func(suscept, snp_cluster), NA),
         resist_sim = ifelse(!is.na(resist), mean_sim_func(resist, snp_cluster), NA),
         inter_sim = ifelse(!is.na(inter), mean_sim_func(inter, snp_cluster), NA),
         sdd_sim = ifelse(!is.na(suscept_dose_dep), mean_sim_func(suscept_dose_dep, snp_cluster), NA))

#write_csv(salmonella_similarity, "data/salmonella_similarity.csv")
#write_csv(ecoli_similarity, "data/ecoli_similarity.csv")
#write_csv(campy_similarity, "data/campy_similarity.csv")

length(unique(campy_similarity$snp_cluster))
